...
<div class="download-category-form">
    ...
    <?= Html::tag('span', '<i class="fa fa-fw fa-question"></i> ' . Yii::t('download_category', 'Enter the name of the download\'s category.'), ['id' => 'hbName', 'class' => 'help-block']) ?>

    <?= $form->field($model, 'status')->dropDownList(DownloadCategory::getStatusData(), ['prompt' => '---', 'aria-describedby' => 'hbStatus']) ?>
    <?= Html::tag('span', '<i class="fa fa-fw fa-question"></i> ' . Yii::t('download_category', 'Please tell us if the download\'s category is active or inactive.'), ['id' => 'hbStatus', 'class' => 'help-block']) ?>
